...
class DTCTecnico(db.Model):
    ...
    NoReferencia = db.Column(db.String(10), primary_key = True, unique = True)
    DTCMovimientosTecnico = relationship("DTCMovimientos", uselist=False, back_populates="DTCTecnicoMovimientos")
...
class DTCMovimientos(db.Model):
    ...
    NoReferencia = db.Column(db.String(10), db.ForeignKey('DTCTecnico.NoReferencia'), primary_key = True , nullable = False)
     #Llave Foranea
    DTCTecnicoMovimientos = relationship("DTCTecnico", back_populates ="DTCMovimientosTecnico")
